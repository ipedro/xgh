...
Automatically select the best agent + model + effort for a task based on learned per-repo performance profiles. Wraps the dispatch skills (`/xgh-gemini`, `/xgh-opencode`) with intelligent routing.
...
| Flag | Purpose |
|------|---------|
| `--agent <name>` | Force a specific agent (gemini, opencode) — skip agent selection |
...
If the model name doesn't match any prefix pattern, require `--agent` and tell the user: "Can't infer agent from model name `<name>`. Pass `--agent gemini|opencode`."
...
# prefs.get("fallback_agent") → e.g. "xgh:gemini"
...
**If cold start (no data for this archetype):** Use project preferences (`preferences.dispatch.*`) if available. Otherwise pick the first available agent from: gemini, opencode (in install order via `bash scripts/detect-agents.sh`). Do NOT pass `-m` flag — let the CLI use its own default model.
...
- **Over-routing.** If the user explicitly invokes `/xgh-gemini` or `/xgh-opencode`, do NOT intercept. Those are direct dispatch — bypass the router.
